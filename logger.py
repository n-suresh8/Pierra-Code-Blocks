...
def serial_thread(port_name, thread_id):
    ...
    file_name = ""
    f = 0
    next_log_time = time.time() #added to control logging interval
    while 1:
        if serial_con.in_waiting:
            line = serial_helper.read_line(serial_con)
            print(line) #was commented
            tokens = line.split(',')
            values_only = tokens[1:-1:2]
            data_line = ",".join(values_only)
            
            with open(f"logs/raw_output_{thread_id}.txt", "w") as raw_file:
                raw_file.write(data_line + "\n")
            ...
            for index2, value in enumerate(serial_values):
                if value in valid_keys:
                    print(serial_values[index2 + 1]) # was commented
                    parsed_values[value] = serial_values[index2 + 1]

            if time.time() >= next_log_time:
                next_log_time += 10 #schedule next log 10 seconds later
